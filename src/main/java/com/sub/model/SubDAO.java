...
public class SubDAO implements SubDAO_interface {
	...
	private static final String INSERT_STMT = "INSERT INTO subscribe (u_id,coach_id,sub_status) VALUES (?, ?, ?)";
	private static final String GET_ALL_STMT = "SELECT su_id,u_id,coach_id,sub_status FROM subscribe order by su_id";
	private static final String GET_ONE_STMT = "SELECT su_id,u_id,coach_id,sub_status FROM subscribe where u_id = ?";
	private static final String DELETE = "DELETE FROM subscribe where su_id = ?";
	...
	public void insert(SubVO subVO) {
		...
		try {
			...
			pstmt.setInt(1, subVO.getuId());
			pstmt.setInt(2, subVO.getCoachId());
			pstmt.setInt(3, subVO.getSubStatus());
